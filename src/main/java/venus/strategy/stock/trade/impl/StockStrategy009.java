 ...
 */
@Component
public class StockStrategy009 implements StockTradeStrategy{
	@Autowired
	...
	private boolean pri(StockData stockData){
		...
		int n=strategyResultMapper.findCount(stockDay.getCode(), "stockStrategy002", startStockDay.getDt(), stockDay.getDt());
			...
			for(int x=2;x<=15;x++){
				for(int y=2;y<=15;y++){
					...
					fixedThreadPool.execute(new Runnable() {
						public void run() {
							strategySimulationTask.simulate(stockDay.getCode(), "stockStrategy002", startStockDay.getDt(), stockDay.getDt(), ii+"#"+jj);
						}
					});
				}
			}
		...
		while(true){
			n=strategyResultMapper.findCount(stockDay.getCode(), "stockStrategy002", startStockDay.getDt(), stockDay.getDt());
			if(n==196){
				break;
			}
			try {
		...
		StrategyResult strategyResult=strategyResultMapper.findMaxProfit(stockDay.getCode(), "stockStrategy002", startStockDay.getDt(), stockDay.getDt());
