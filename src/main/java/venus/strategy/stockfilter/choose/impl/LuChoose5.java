...
@Component
public class LuChoose5 implements LuChoose{
	Logger logger=Logger.getLogger(LuChoose5.class);
...
//				if(!zongshizhiStockFilter.filter(code, "<100"))continue;
				if(!hangyeStockFilter.filter(code, "1,!=医药生物&&!=房地产"))continue;
//				if(!shiyinglvttmStockFilter.filter(code, "<50"))continue;
				if(!jingzichanshouyilvStockFilter.filter(code, ">8&&<35"))continue;
