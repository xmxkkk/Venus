 ...
 */
@Component
public class LuChoose2 implements LuChoose{
	Logger logger=Logger.getLogger(LuChoose2.class);
	...
	public List<String> choose() {
		...
		try{
			...
			for(Stockinfo stock:stocks){
				...
				//传入合法条件
//				if(!zongshizhiStockFilter.filter(code, "<20000000000D"))continue;
//				if(!hangyeStockFilter.filter(code, "1,!=房地产&&!=金融服务&&!=信息服务"))continue;
//				if(!shiyinglvttmStockFilter.filter(code, "<100"))continue;
				if(!jingzichanshouyilvStockFilter.filter(code, ">1"))continue;
...
//				if(!priceStockFilter.filter(code, "<35"))continue;
//				if(!financerateStockFilter.filter(code, "营业总收入,>10"))continue;
//				if(!betaStockFilter.filter(code, ">2.5"))continue;
//				if(!nameStockFilter.filter(code, "!INST"))continue;
//				if(!yewuStockFilter.filter(code, "IN机器人"))continue;
//				if(!xiadieStockFilter.filter(code, "60,<-10"))continue;
				
				if(!xianjinliuStockFilter.filter(code, "筹资现金,>30"))continue;
//				if(!xianjinliuStockFilter.filter(code, "经营现金,>0"))continue;
				if(!xianjinliuStockFilter.filter(code, "投资现金,>30"))continue;
