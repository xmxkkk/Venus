 ...
 */
@Component
public class LuChoose1 implements LuChoose{
	Logger logger=Logger.getLogger(LuChoose1.class);
	...
	public List<String> choose() {
		...
		try{
			...
			for(Stockinfo stock:stocks){
				...
				//传入合法条件
				if(!zongshizhiStockFilter.filter(code, "<50000000000D"))continue;
//				if(!hangyeStockFilter.filter(code, "1,!=房地产&&!=金融服务&&!=信息服务"))continue;
				if(!shiyinglvttmStockFilter.filter(code, "<60"))continue;
				if(!jingzichanshouyilvStockFilter.filter(code, ">5"))continue;
				if(!top1holderrateStockFilter.filter(code, ">20"))continue;
...
//				if(!dongshizhangStockFilter.filter(code, null))continue;
				...
				if(!financerateStockFilter.filter(code, "营业总收入,>7"))continue;
				if(!financerateStockFilter.filter(code, "净利润,>7"))continue;
